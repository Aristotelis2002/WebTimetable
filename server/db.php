...
function getUserInterests($userId) {
	$db = new Db('interests'); 
	$sql = "SELECT interestType, orderId, date, hour, fn, groupStudent, name, topicId, topic FROM interests JOIN presentations ON presentations.id = presentationId WHERE interests.userId = '$userId'" ;
    $result = $db->query($sql);
	if ($result->rowCount() > 0) {	
        $interestsRow = $result->fetchAll(PDO::FETCH_ASSOC);
        return $interestsRow;
